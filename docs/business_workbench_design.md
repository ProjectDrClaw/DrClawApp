...
| `sendFileToAgent` | `createFileMessageFromFullPath` + send；返回 `sourceUrl` |
...
| 编号 | 议题 | 决策 | 备注 |
|------|------|------|------|
| ADR-1 | 长录音如何发给 Agent | **文件消息 contentType=105** | 不用 103 语音；**先发文件再发**患者说明；文件名与自定义录音标题一致（对齐飞书 `file_name`） |
...
3. `host.sendFileToAgent(filePath, fileName)`（**105 文件**；`fileName` 与自定义录音标题一致，如 `12床_查房纪要_180s.m4a`，对齐飞书保留 `file_name`）。  
4. `host.sendTextToAgent(患者基本信息补充文案)`（须在文件之后；不写下载地址/文件名等技术字段）。  
...
| 情况 | 处理 |
|------|------|
| 文本成功、文件失败 | `status=failed`；重试时**仍整单重发**（患者说明文本 + 文件） |
...
空字段省略。录音配套文案只补充患者基本信息（标明录音对应谁），不写下载地址/文件名等技术字段。
...
| `以下为该录音对应的患者信息…` | 工作台/工具箱发录音 | 仅患者字段补充；音频走文件消息 |
